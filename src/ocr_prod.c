// ocr_prod.c : OCR en production
// GRID:    grid/cells/cell_<id>.png   (id = 0..N*N-1)
// WORDS:   wordlist/cells/word_<w>/letter_<k>.png
// OUTPUT:  grid.txt (ligne par ligne), words.txt (mot par ligne)
                                ...
                                double PTH, double MTH)
{
    ...
    // top1/top2
    int best = 0, second = 1;
    if (net->a2[1] > net->a2[0]) { best = 1; second = 0; }

    for (int k = 2; k < 26; ++k) {
        if (net->a2[k] > net->a2[best]) { second = best; best = k; }
        else if (net->a2[k] > net->a2[second]) { second = k; }
    }
    ...
    while ((de = readdir(d)) != NULL) {
        ...
        if (sscanf(de->d_name, "cell_%d.png", &id) == 1) {
            if (*out_n >= cap) {
                ...
                if (!tmp) { free(arr); closedir(d); return NULL; }
                arr = tmp;
            }
            arr[*out_n].id = id;
            snprintf(arr[*out_n].path, MAX_PATH, "%s/%s", dirpath, de->d_name);
            (*out_n)++;
        }
    }
    closedir(d);

    if (*out_n == 0) { free(arr); return NULL; }
...
static int write_grid_txt(Network *net) {
    ...
    // Déduire taille de grille (supposée carrée)
    int side = (int)(sqrt((double)n) + 0.5);
    if (side * side != n) {
        fprintf(stderr, "Grid cells count (%d) is not a perfect square.\n", n);
        fprintf(stderr, "Cannot infer grid size automatically.\n");
        free(cells);
        return 0;
    }

    // On remplit une grille side*side, en supposant cell_id = row*side + col
    char *grid = (char*)malloc((size_t)side * side);
    if (!grid) { free(cells); return 0; }
    for (int i = 0; i < side * side; ++i) grid[i] = '?';

    for (int i = 0; i < n; ++i) {
        int id = cells[i].id;
        if (id < 0 || id >= side * side) continue;
        grid[id] = predict_char_grid(net, cells[i].path);
    }

    FILE *f = fopen(OUT_GRID_TXT, "w");
    if (!f) { perror(OUT_GRID_TXT); free(grid); free(cells); return 0; }
...
// -------- WORDLIST: word_<w>/letter_<k>.png --------
...
static int write_words_txt(Network *net) {
    ...
    if (!out) { perror(OUT_WORD_TXT); return 0; }

    // On parcourt word_0, word_1, ... jusqu'à ce qu'un dossier manque
    for (int w = 0; ; ++w) {
        char wdir[MAX_PATH];
        snprintf(wdir, sizeof(wdir), "%s/word_%d", WORDS_ROOT, w);

        if (!dir_exists(wdir)) {
            // stop dès qu'on ne trouve plus le prochain word_<w>
            break;
        }

        // Lire letter_0.png, letter_1.png, ...
        for (int k = 0; ; ++k) {
